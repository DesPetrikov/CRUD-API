...
import { ENDPOINT_NOT_EXIST, SERVER_ERROR, UNSUPPORTED_METHOD } from './constants.js';
...
export const server = http.createServer(async (req: IncomingMessage, res: ServerResponse) => {
  ...
  try {
    if (req.url.startsWith('/api/users') && !perspectiveInIT) {
      if (req.method === 'GET') {
        responseBody = userId ? getUserController(userId, usersList) : usersList;
        ...
        const { userIdx, updatedUser } = await putController(req, userId, usersList);
